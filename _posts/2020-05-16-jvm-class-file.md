...
一个很重要、又有点复杂的地方。 这块有个地方比较迷啊，00 03（行：0000000，列：8~9）显示”代码示例“的常量池中有50（0033换算成十进制再减一）个常量，但Idea中的插件显示”代码示例“中有56个常量，而且每个都枚举了出来，在此先按照56来算吧。

这块地方主要存放两大类常量：**字面量** 和 **符号引用**：

字面量就相当于java概念中的常量，比如：文本字符串、被声明为final的常量值等。

符号引用可以理解为：按”一定规则“对java文件中的类、字段、方法、接口等信息进行了另一种”描述“。（虚拟机只能识别这种描述，不能识别开发者写的java形式的类）。符号引用主要包含以下几类：
